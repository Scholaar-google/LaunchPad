...
Handles partial missing results — detects conflicts between any available agent pairs.
"""
...
def detect_conflicts(
    ...
    if feasibility is not None and resource is not None:
        ...
        if "可行" in feas_conclusion and ("不足" in res_conclusion or "缺乏" in res_conclusion):
            conflicts.append(
                ConflictItem(
                    ...
                    conclusion_a=str(feasibility["conclusion"]),
                    source_b="resource_agent",
                    conclusion_b=str(resource["conclusion"]),
                    resolution_paths=[
                        "缩小项目范围以匹配现有资源",
                        "申请额外资源预算以支撑可行性目标",
                        "分阶段交付，降低首期资源需求",
    ...
    if feasibility is not None and risk is not None:
        feas_conclusion = feasibility.get("conclusion", "").lower()
        ...
        if "可行" in feas_conclusion and (risk_score >= 7.0 or "高" in risk_conclusion):
            conflicts.append(
                ConflictItem(
                    ...
                    conclusion_a=str(feasibility["conclusion"]),
                    source_b="risk_agent",
                    conclusion_b=str(risk["conclusion"]),
                    resolution_paths=[
                        "制定详细风险缓解计划后推进",
                        "先执行技术预研/POC降低不确定性",
                        "调整技术方案以规避主要风险",
    ...
    # Rule 4: Risk mentions compliance/regulatory issues but feasibility doesn't cover
    if feasibility is not None and risk is not None:
