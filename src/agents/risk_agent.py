...
from src.prompts.templates import RISK_PROMPT
...
async def run_risk(state: GlobalState) -> dict[str, Any]:
    ...
    system_prompt = "你是一个风险评估专家，请以JSON格式输出。"
...
async def run_risk_with_timeout(state: GlobalState, timeout: int = 60) -> dict[str, Any]:
    try:
        return await run_risk(state)
    except Exception as e:
        logger.error("risk_timeout_or_error", error=str(e))
