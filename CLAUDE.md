...
| 等级 | 定义 | 示例 |
|------|------|------|
| **P0** | 系统无法运行或编译失败 | TypeScript 编译错误、Python 导入错误、启动即崩溃、死循环阻塞 |
| **P1** | 严重影响体验，但核心功能仍可用 | 数据重复导致 Token 浪费、并行变串行性能骤降、状态损坏 |
| **P2** | 功能正常，有明显瑕疵、边界 case 异常、小范围影响 | LLM 返回 null 导致静默吞错、未初始化字段、缺少版本检查 |
| **P3** | 界面/文案/样式瑕疵、冗余代码，不影响功能 | 字体不跨平台、未使用的依赖、空节点、冗余类型转换 |

发现 Bug 后必须按以上等级分类并写入提交信息，P0/P1 优先修复。
