...
7. **Agent 独立 LLM 配置**：每个 Agent 通过独立的 `AGENT_LLM_CONFIG_<NAME>` 环境变量（JSON 格式）配置 LLM 厂商、API Key、模型。未配置的字段自动回退到全局默认值
   - 示例: `AGENT_LLM_CONFIG_SYNTHESIS={"api_key":"sk-ant-xxx","base_url":"https://api.anthropic.com","model":"claude-sonnet-4-20250514"}`
   - 实现: `llm_config.py` 中 `_get_agent_client(agent_name)` 按 agent 名称读取对应环境变量，缓存 client 实例
